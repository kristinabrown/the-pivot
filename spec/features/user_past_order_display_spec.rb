...
feature "an authenticated user's past orders" do 
  before(:each) do 
  beer1 = Beer.create({name: "Yeti", state: true, description: "big, roasty malt flavor with rich caramel and toffee notes", price: 606, created_at: Date.parse("2012-03-25 09:54:09 UTC"), updated_at: Date.parse("2012-03-25 09:54:09 UTC")})
  beer2 = Beer.create({name: "Chocolate Stout", state: true, description: "full-bodied flavor with hints of rich oatmeal and dark chocolate", price: 612, created_at: Date.parse("2012-03-25 09:54:09 UTC"), updated_at: Date.parse("2012-03-25 09:54:09 UTC")})
  ...
  # @order.beers.create(order_id: @order.id, beer_id: beer1.id, quantity: 1)
  # @order.beers.create(order_id: @order.id, beer_id: beer2.id, quantity: 1)
  visit root_path
  click_link "Login"
...
feature "a user's past orders" do 
  ...
  xscenario "will not allow access to admin functions" do 
